...
**Before (v0.1.x)**:
- Single package: `skill-search` (~280 MB with all dependencies)
- stdio MCP server only
- All dependencies downloaded on first `uvx` run (could timeout in Cursor)

**After (v1.0.0)**:
- **Frontend**: `skill-search` (~15 MB, lightweight proxy)
- **Backend**: `skill-search-backend` (~250 MB, heavy server)
...
```python
# Before (v0.1.x)
from src.skill_search.search_engine import SkillSearchEngine
from src.skill_search.skill_loader import load_skills_from_github

# After (v1.0.0)
from skill_search_backend.search_engine import SkillSearchEngine
from skill_search_backend.skill_loader import load_skills_from_github
```
...
pip install skill-search skill-search-backend
...
**After (v1.0.0)**:
- **Frontend** (`skill-search`): Lightweight proxy (~15 MB)
- **Backend** (`skill-search-backend`): Heavy server (~250 MB)
...
```bash
uvx skill-search --config custom.json --verbose
...
docker run -p 8080:8765 skill-search-backend
```
...
4. **uvx handles everything**: Frontend spawns backend via `uvx skill-search-backend`
...
- ✅ Backend package (`skill-search-backend`) complete
- ✅ Frontend package (`skill-search`) complete
...
```bash
cd /Users/haoxuanl/kdense/skill-search/packages/backend

# Verify build
ls -lh dist/
# Should see:
# - skill_search_backend-1.0.0-py3-none-any.whl (24KB)
# - skill_search_backend-1.0.0.tar.gz (36KB)

# Publish to PyPI
uv publish

# Verify on PyPI
open https://pypi.org/project/skill-search-backend/
```
...
```bash
cd /Users/haoxuanl/kdense/skill-search/packages/frontend

# Verify build
ls -lh dist/
# Should see:
# - skill_search-1.0.0-py3-none-any.whl (11KB)
# - skill_search-1.0.0.tar.gz (9.1KB)

# Publish to PyPI
uv publish

# Verify on PyPI
open https://pypi.org/project/skill-search/
```

**Step 4: Create GitHub Release**

```bash
cd /Users/haoxuanl/kdense/skill-search
...
```bash
# Clean environment
uv cache clean skill-search
uv cache clean skill-search-backend
...
#### Backend ✅ Published
- Package: `skill-search-backend` v1.0.0
- PyPI: https://pypi.org/project/skill-search-backend/
- Status: Live and working
- Verified: `uvx skill-search-backend --help` works

#### Frontend ⏳ Ready to Publish
- Package: `skill-search` v1.0.0
...
2. ✅ **Simple UX Maintained**: Still just `uvx skill-search`
