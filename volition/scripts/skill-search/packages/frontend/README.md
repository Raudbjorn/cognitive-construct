...
uvx skill-search --config my-config.json

# Verbose logging
uvx skill-search --verbose

# Custom backend port (advanced)
uvx skill-search --port 9000
```
...
uvx skill-search --remote https://skills.k-dense.ai/mcp
```
...
The backend (`skill-search-backend`) is auto-installed on first use.
...
cd skill-search/packages/frontend
...
- **skill-search-backend** (Backend): Heavy server with vector search
