...
def test_frontend_cold_start_under_cursor_timeout():
    ...
    subprocess.run(["uv", "cache", "clean", "skill-search"], capture_output=True)

    print("\nTesting frontend cold start (cache cleared)...")
    start_time = time.time()

    result = subprocess.run(
        ["uvx", "skill-search", "--help"],
        capture_output=True,
        text=True,
...
def test_frontend_warm_start_very_fast():
    ...
    subprocess.run(
        ["uvx", "skill-search", "--help"], capture_output=True, timeout=30
    )
...
def test_backend_cold_start_time():
    ...
    # Clear backend cache
    subprocess.run(
        ["uv", "cache", "clean", "skill-search-backend"], capture_output=True
    )
...
def test_backend_warm_start():
    ...
    # Ensure cached
    subprocess.run(
        ["uvx", "skill-search-backend", "--help"], capture_output=True, timeout=60
    )
...
def test_package_sizes():
    ...
    frontend_wheel = project_root / "packages/frontend/dist/skill_search-1.0.5-py3-none-any.whl"
    ...
    backend_wheel = project_root / "packages/backend/dist/skill_search_backend-1.0.6-py3-none-any.whl"
