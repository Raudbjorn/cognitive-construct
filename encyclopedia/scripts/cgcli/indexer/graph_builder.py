...
class GraphBuilder:
    ...
    def _pre_scan_for_imports(self, files: list[Path]) -> dict:
        ...
        _SCAN_MAP = {
            '.py': ('python', 'pre_scan_python'),
            '.ipynb': ('python', 'pre_scan_python'),
            '.js': ('javascript', 'pre_scan_javascript'),
            '.jsx': ('javascript', 'pre_scan_javascript'),
            '.mjs': ('javascript', 'pre_scan_javascript'),
            '.cjs': ('javascript', 'pre_scan_javascript'),
            '.go': ('go', 'pre_scan_go'),
            '.ts': ('typescript', 'pre_scan_typescript'),
            '.tsx': ('typescriptjsx', 'pre_scan_typescript'),
            '.cpp': ('cpp', 'pre_scan_cpp'),
            '.h': ('cpp', 'pre_scan_cpp'),
            '.hpp': ('cpp', 'pre_scan_cpp'),
            '.hh': ('cpp', 'pre_scan_cpp'),
            '.rs': ('rust', 'pre_scan_rust'),
            '.c': ('c', 'pre_scan_c'),
            '.java': ('java', 'pre_scan_java'),
            '.rb': ('ruby', 'pre_scan_ruby'),
            '.cs': ('csharp', 'pre_scan_csharp'),
            '.kt': ('kotlin', 'pre_scan_kotlin'),
            '.scala': ('scala', 'pre_scan_scala'),
            '.sc': ('scala', 'pre_scan_scala'),
            '.swift': ('swift', 'pre_scan_swift'),
        }

        for ext, lang_files in files_by_lang.items():
            if ext not in _SCAN_MAP:
                continue
            module_name, func_name = _SCAN_MAP[ext]
            try:
                mod = __import__(
                    f"cgcli.indexer.languages.{module_name}",
                    fromlist=[func_name],
                )
                scan_fn = getattr(mod, func_name)
                imports_map.update(scan_fn(lang_files, self.parsers[ext]))
            except (ImportError, AttributeError):
                debug_log(f"No pre-scan for {ext} ({module_name}.{func_name})")
