        ...
        inputSchema={
            ...
            "properties": {
                ...
                "top_k": {
                    "type": "integer",
                    "description": (
                        "Number of skills to return (default: 3). Higher values "
                        "provide more options but may include less relevant results."
                    ),
                    "default": 3,
                    "minimum": 1,
                    "maximum": 20,
                ...
                "list_documents": {
                    "type": "boolean",
                    "description": (
                        "Include a list of available documents (scripts, references, "
                        "assets) for each skill (default: True)"
                    ),
                    "default": True,
                },
            },
...
class MCPProxy:
    ...
    def __init__(self, backend_args: list[str]) -> None:
    ...
    async def _start_backend_async(self) -> None:
        ...
        try:
            ...
            # Ensure backend is running (returns Result type)
            result = await self.backend_manager.ensure_backend_running(
                self.backend_args
            )

            if result.is_err():
                logger.error(f"Backend failed to start: {result.error.message}")
                return
    ...
    async def _connect_to_backend(self, url: str) -> None:
        ...
        """
        try:
            # Use AsyncExitStack to keep the connection alive
