"""Graph builder — adapted from CGC tools/graph_builder.py.

SCIP indexer and FalkorDB support have been stripped.
"""
import asyncio
import os
import pathspec
...
# Labels that get the secondary :Searchable label for vector index support
_SEARCHABLE_LABELS = {"Function", "Class", "Variable"}
...
class GraphBuilder:
    ...
    def create_schema(self):
        ...
        with self.driver.session() as session:
            try:
                ...
                # Vector index for semantic search (requires Neo4j >= 5.11)
                try:
                    session.run("""
                        CREATE VECTOR INDEX code_vector_index IF NOT EXISTS
                    ...
                    warning_logger(f"Vector index creation skipped (Neo4j may be < 5.11): {vec_err}")
        ...
        with self.driver.session() as session:
            ...
            for item_data, label in item_mappings:
                for item in item_data:
                    ...
                    # Add :Searchable secondary label for vector-indexed node types
                    effective_label = f"{label}:Searchable" if label in _SEARCHABLE_LABELS else label
                    query = f"""
                        MATCH (f:File {{path: $path}})
                        MERGE (n:{effective_label} {{name: $name, path: $path, line_number: $line_number}})
                        SET n += $props
                        MERGE (f)-[:CONTAINS]->(n)
                    """
    ...
    def _embed_nodes(self, repo_name: str, job_id: str | None = None) -> None:
        """Compute and store embeddings for Searchable nodes missing them.

        Failures never block indexing — the entire phase is wrapped in
        try/except and degrades silently.
        """
        ...
        try:
            ...
            with self.driver.session() as session:
                records = session.run(
                    ...
                    RETURN elementId(n) AS eid, labels(n) AS lbls,
                           n.name AS name, n.docstring AS docstring,
                           n.source AS source, n.path AS path
                    """
                ).data()
            ...
            if job_id:
                self.job_manager.update_job(job_id, current_file=f"Embedding {len(records)} nodes")

            texts: list[str] = []
            eids: list[str] = []
            for rec in records:
                node_type = next(
                    (l for l in rec["lbls"] if l != "Searchable"), "Code"
                )
            ...
            # Write embeddings back in batches of 100
            batch_size = 100
            with self.driver.session() as session:
                for i in range(0, len(eids), batch_size):
                    batch = [
                        {"eid": eids[j], "embedding": vectors[j].tolist()}
                        for j in range(i, min(i + batch_size, len(eids)))
                    ]
                    session.run(
                        """
                        UNWIND $batch AS p
                        MATCH (n) WHERE elementId(n) = p.eid
                        SET n.embedding = p.embedding
                        """,
                        batch=batch,
                    )

            info_logger(f"Embedded {len(eids)} nodes for repo '{repo_name}'")
    ...
    async def build_graph_from_path_async(
        ...
        try:
            ...
            # Compute embeddings for semantic vector search (non-blocking)
            self._embed_nodes(repo_name, job_id)
