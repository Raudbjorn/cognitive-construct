...
- **Frontend** (`skill-search`): Lightweight MCP proxy that starts instantly
- **Backend** (`skill-search-backend`): Heavy server with vector search and skill loading
...
**Caching Mechanism**:
- Cache location: System temp directory (`/tmp/skill_search_cache/`)
...
4. **Disk Cache**: Persist to `/tmp/skill_search_cache/documents/` for future runs
...
- Cache location: `/tmp/skill_search_cache/state/`
...
### Backend Package (`skill-search-backend`)
