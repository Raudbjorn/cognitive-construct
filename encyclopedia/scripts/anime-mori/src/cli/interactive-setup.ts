...
export class InteractiveSetup {
  ...
  async run(): Promise<void> {
    console.log('🚀 Welcome to Anime Mori Interactive Setup!');
    console.log('This wizard will help you configure Anime Mori for your project.\n');
    ...
    try {
      ...
      console.log('   1️⃣  Run `anime-mori server` to start the MCP server');
      console.log('   2️⃣  Add Anime Mori to your Claude Desktop/Code configuration');
  ...
  private async collectConfiguration(): Promise<SetupConfig> {
    ...
    console.log('\n📊 Vector Embeddings:');
    console.log('   Anime Mori uses local embeddings (transformers.js) for semantic code search.');
  ...
  private async createConfiguration(config: SetupConfig): Promise<void> {
    ...
    // Create .anime-mori directory
    const configDir = join(config.projectPath, '.anime-mori');
  ...
  private async performInitialLearning(config: SetupConfig): Promise<void> {
    ...
    let storage: SurrealStorage | null = null;
    ...
    try {
      ...
      storage = await SurrealStorage.create(join(config.projectPath, 'anime-mori.surql'));
      semanticEngine = new SemanticEngine(storage);
      patternEngine = new PatternEngine(storage);
      ...
      console.error('\n💡 You can run learning later with: `anime-mori learn`');
      ...
      if (storage) {
        try {
          ...
          console.warn('Warning: Failed to close storage:', error);
        }
      }
    }
  }
  ...
  private async updateGitignore(projectPath: string): Promise<void> {
    ...
    if (existsSync(gitignorePath)) {
      ...
      if (!content.includes('anime-mori.surql')) {
        ...
        console.log('\n✅ Updated .gitignore with Anime Mori entries');
      }
    } else {
      writeFileSync(gitignorePath, gitignoreEntries + '\n');
      console.log('\n✅ Created .gitignore with Anime Mori entries');
    }
  }
