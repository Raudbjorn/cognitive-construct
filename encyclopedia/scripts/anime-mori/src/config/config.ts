/**
 * Configuration management for Anime Mori
...
export interface AnimeMoriConfig {
...
const DEFAULT_CONFIG: AnimeMoriConfig = {
  database: {
    filename: 'anime-mori.surql', // SurrealDB SurrealKV storage
...
export class ConfigManager {
  ...
  private config: AnimeMoriConfig;
  ...
  getDatabasePath(projectPath?: string): string {
    ...
    if (filename.includes('/') || filename.includes('\\')) {
      Logger.warn(
        '⚠️  Warning: ANIME_MORI_DB_FILENAME contains path separators.\n' +
        `   Current: "${filename}"\n` +
        '   This may cause issues. Consider using a simple filename.\n' +
        `   The database directory is determined by the project path: ${basePath}\n` +
        '   Example: Set ANIME_MORI_DB_FILENAME="anime-mori.surql" instead of a path.'
      );
    }
   ...
   */
  updateConfig(partial: Partial<AnimeMoriConfig>): void {
  ...
  private loadFromEnvironment(): void {
    ...
    if (process.env.ANIME_MORI_DB_FILENAME) {
      this.config.database.filename = process.env.ANIME_MORI_DB_FILENAME;
    }
    
    if (process.env.ANIME_MORI_STORAGE_DIR) {
      this.config.database.storageDir = process.env.ANIME_MORI_STORAGE_DIR;
    }
    
    // Performance configuration
    if (process.env.ANIME_MORI_BATCH_SIZE) {
      this.config.performance.batchSize = parseInt(process.env.ANIME_MORI_BATCH_SIZE, 10);
    }
    
    if (process.env.ANIME_MORI_MAX_CONCURRENT) {
      this.config.performance.maxConcurrentFiles = parseInt(process.env.ANIME_MORI_MAX_CONCURRENT, 10);
    }

    // API configuration
    if (process.env.ANIME_MORI_REQUEST_TIMEOUT) {
      this.config.api.requestTimeout = parseInt(process.env.ANIME_MORI_REQUEST_TIMEOUT, 10);
    }
    
    // Logging configuration
    if (process.env.ANIME_MORI_LOG_LEVEL) {
      const level = process.env.ANIME_MORI_LOG_LEVEL.toLowerCase();
      if (['error', 'warn', 'info', 'debug'].includes(level)) {
        this.config.logging.level = level as any;
      }
    }
    
    if (process.env.ANIME_MORI_PERFORMANCE_LOGGING === 'true') {
  ...
  getConfigurationHelp(): string[] {
    return [
      ...
      '  ANIME_MORI_DB_FILENAME - Database filename (default: anime-mori.surql)',
      '  ANIME_MORI_STORAGE_DIR - Directory to store database files (default: project directory)',
      '  ANIME_MORI_BATCH_SIZE - File processing batch size (default: 50)',
      '  ANIME_MORI_MAX_CONCURRENT - Max concurrent file operations (default: 10)',
      '  ANIME_MORI_REQUEST_TIMEOUT - API request timeout in ms (default: 30000)',
      '  ANIME_MORI_LOG_LEVEL - Logging level: error|warn|info|debug (default: info)',
      '  ANIME_MORI_PERFORMANCE_LOGGING - Enable performance logging (default: false)',
      '',
