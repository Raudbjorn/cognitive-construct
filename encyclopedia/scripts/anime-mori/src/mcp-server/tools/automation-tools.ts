...
import type { SurrealStorage } from '../../storage/surreal-storage.js';
...
export class AutomationTools {
    ...
    return [
      {
        ...
        description: 'Automatically learn from codebase if intelligence data is missing or stale. Call this first before using other Anime-Mori tools - it\'s a no-op if data already exists. Includes project setup and verification. Perfect for seamless agent integration.',
        inputSchema: {
          type: 'object',
          properties: {
            path: {
  ...
  async autoLearnIfNeeded(args: {
    ...
    if (includeSetupSteps) {
      ...
      const schemaVersion = await this.storage.getSchemaVersion();
      setupSteps!.push({
        step: 'database_init',
        status: 'completed',
        message: 'Database initialized and schema applied',
        details: {
    ...
    if (skipLearning) {
      if (includeSetupSteps) {
        ...
        return {
          ...
          projectPath,
          steps: setupSteps,
          message: '✅ Quick setup completed! Anime Mori is ready for AI agent use.',
  ...
  async getLearningStatus(args: { path?: string }): Promise<any> {
    ...
    try {
      ...
      const patterns = await this.storage.getDeveloperPatterns();
      ...
      return {
        ...
        lastLearningTime: hasIntelligence ? await this.getLastLearningTime() : null,
  ...
  async quickSetup(args: {
    ...
    try {
      ...
      // Step 2: Database initialization (automatic)
      const setupSchemaVersion = await this.storage.getSchemaVersion();
      steps.push({
        step: 'database_init',
        status: 'completed',
        message: 'Database initialized and schema applied',
        details: {
          version: setupSchemaVersion,
          tablesReady: true
        }
      });
      ...
      return {
        ...
        steps,
        message: success
          ? '✅ Quick setup completed! Anime Mori is ready for AI agent use.'
  ...
  private async getLastLearningTime(): Promise<string | null> {
    // Simple heuristic - get the latest created_at from semantic_concepts or developer_patterns
    try {
      const allConcepts = await this.storage.getSemanticConcepts();
      const latestConcept = allConcepts
        .sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];

      const allPatterns = await this.storage.getDeveloperPatterns();
      const latestPattern = allPatterns
        .sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
