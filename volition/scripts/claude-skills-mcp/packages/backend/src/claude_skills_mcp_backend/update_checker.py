...
class UpdateResult:
    """Result of checking for updates (immutable).
    ...
    changed_sources : tuple[dict[str, Any], ...]
        Tuple of source configs that have changes.
    api_calls_made : int
...
@dataclass
class MutableUpdateResult:
    """Mutable version for building UpdateResult during checking."""
    ...
    def to_immutable(self) -> UpdateResult:
        """Convert to immutable UpdateResult."""
        return UpdateResult(
            has_updates=self.has_updates,
            changed_sources=tuple(self.changed_sources),
...
class LocalSourceTracker:
    ...
    def check_for_updates(self, source_config: dict[str, Any]) -> bool:
        ...
        for skill_file in skill_files:
            try:
                ...
                if file_key not in last_mtimes:
                    ...
                    has_changes = True
                elif abs(last_mtimes[file_key] - mtime) > 0.001:
        ...
        if has_changes or not last_mtimes:
            ...
            if not last_mtimes:
                logger.info(
                    f"First check for local path {path}, "
                    f"tracking {len(skill_files)} files"
                )
                return False
...
class UpdateChecker:
    ...
    def check_for_updates(
        self, skill_sources: list[dict[str, Any]]
    ) -> UpdateResult:
        ...
        -------
        UpdateResult
            Immutable result containing changed sources and metadata.
        """
        result = MutableUpdateResult()
