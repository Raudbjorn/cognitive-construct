...
import { ErrorFactory, MCPErrorUtils, ErrorUtils, AnimeMoriError, MCPErrorCode } from '../../utils/error-types.js';
...
export class CoreAnalysisTools {
  ...
  constructor(
    ...
    this.intelligenceTools = new IntelligenceTools(semanticEngine, patternEngine, storage);
  }
  ...
  async getFileContent(args: { path: string }): Promise<{
    ...
    try {
      ...
      if (error instanceof AnimeMoriError) {
        throw error;
      }
      
      // Convert to proper AnimeMori error with MCP compliance
  ...
  private async buildIntelligentDocumentation(analysis: any, options: DocOptions): Promise<string> {
    ...
    sections.push(`\n*🤖 Generated on ${analysis.analysisTimestamp.toLocaleString()} by Anime Mori AI*`);
