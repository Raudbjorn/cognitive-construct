...
class BackendManager:
    """Manages skill-search-backend installation and lifecycle.
    ...
    def check_backend_available(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["uvx", "--help"], capture_output=True, timeout=5
            )
    ...
    async def start_backend(
        ...
        -------
        Result[str, BackendError]
            Ok with backend URL or Err with BackendError.
        """
        logger.info(
        ...
        cmd = f"PYTHONUNBUFFERED=1 uvx skill-search-backend {args_str}"
        ...
        try:
            ...
            self.backend_url = (
                f"http://{self.backend_host}:{self.backend_port}/mcp"
            )

            wait_result = await self._wait_for_health(timeout=300)
            if wait_result.is_err():
                return Err(wait_result.error)

            logger.info(f"Backend ready at {self.backend_url}")
            return Ok(self.backend_url)

        except Exception as e:
            error_msg = f"Failed to start backend server: {e}"
            logger.error(error_msg)
            return Err(BackendError(message=error_msg, cause=str(e)))

    async def _consume_stream(
        self, stream: asyncio.StreamReader | None, prefix: str
    ) -> None:
        ...
        try:
            ...
            logger.error(
                f"Stream consumer {prefix} error after {line_count} lines: {e}"
            )
    ...
    async def _wait_for_health(
        ...
        -------
        Result[None, BackendError]
            Ok if healthy, Err with BackendError if timeout.
        """
        ...
        while True:
                ...
                async with httpx.AsyncClient(timeout=5.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if loading_complete and skills_loaded > 0:
                            logger.info(
                                f"Backend health check passed "
                                f"({skills_loaded} skills loaded)"
                            )
                            return Ok(None)
                        elif loading_complete and skills_loaded == 0:
                            # Still no skills after loading complete
                            last_error = (
                                "Backend loaded but no skills found "
                            ...
                            if int(elapsed) % 30 == 0 and int(elapsed) > 0:
                                logger.info(
                                    f"Still loading skills... "
                                    f"({skills_loaded} loaded so far)"
                                )
            ...
            if elapsed > timeout:
                return Err(
                    BackendError(
                        message=f"Backend failed to start within {timeout}s",
                        cause=last_error,
                    )
                )
    ...
    async def ensure_backend_running(
        self, backend_args: list[str]
    ) -> Result[str, BackendError]:
    ...
    async def cleanup(self) -> None:
        ...
        if self.backend_process:
            ...
            try:
                logger.info(
                    f"Terminating backend process group "
                    f"(PID: {self.backend_process.pid})"
                )

                try:
                    # Kill the whole process group (negative PID)
                    os.killpg(
                        os.getpgid(self.backend_process.pid), signal.SIGTERM
                    )
                ...
                try:
                    ...
                    try:
                        os.killpg(
                            os.getpgid(self.backend_process.pid), signal.SIGKILL
                        )
                    except ProcessLookupError:
                        pass
