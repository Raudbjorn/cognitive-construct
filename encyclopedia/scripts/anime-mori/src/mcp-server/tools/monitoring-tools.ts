...
export class MonitoringTools {
  ...
  get tools(): Tool[] {
    return [
      ...
      {
        name: 'health_check',
        description: 'Verify Anime-Mori setup and configuration for a project. Checks database accessibility, project structure, and API keys.',
        inputSchema: {
          type: 'object',
          properties: {
            path: {
  ...
  async getPerformanceStatus(args: {
    ...
    try {
      ...
      if (existsSync(dbPath)) {
        ...
        concepts = await this.storage.getSemanticConcepts();
        const conceptsMs = Date.now() - conceptQueryStart;

        const patternQueryStart = Date.now();
        patterns = await this.storage.getDeveloperPatterns();
  ...
  private async runQuickBenchmark(): Promise<any> {
    ...
    await this.storage.getSemanticConcepts();
    benchmark.conceptQuery = Date.now() - conceptStart;

    // Pattern query benchmark
    const patternStart = Date.now();
    await this.storage.getDeveloperPatterns();
  ...
  async healthCheck(args: { path?: string }): Promise<{
    ...
    try {
      const dbPath = config.getDatabasePath(projectPath);
      if (existsSync(dbPath)) {
        const tempStorage = await SurrealStorage.create(dbPath);
        try {
          const concepts = await tempStorage.getSemanticConcepts();
          const patterns = await tempStorage.getDeveloperPatterns();
