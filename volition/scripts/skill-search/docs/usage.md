...
```json
{
  "mcpServers": {
    "claude-skills": {
      "command": "uvx",
      "args": ["skill-search", "--config", "/path/to/config.json"]
    }
  }
}
```
...
uv run skill-search --config test-config.json --verbose
```
...
- Location: `/tmp/skill_search_cache/{md5}.json`
...
- Location: `/tmp/skill_search_cache/documents/{md5}.cache`
