...
export class IntelligenceTools {
  constructor(
    ...
    private storage: SurrealStorage
  ) {}
  ...
  get tools(): Tool[] {
    return [
      ...
      },
      {
        ...
        description: 'Let AI agents save discovered insights (bug patterns, optimizations, best practices) back to Anime-Mori for future reference. Use this when you discover a recurring pattern, potential bug, or refactoring opportunity that other agents/sessions should know about. Creates organizational memory across conversations.',
        inputSchema: {
          type: 'object',
          properties: {
            type: {
  ...
  async getSemanticInsights(args: {
    ...
    const concepts = await this.storage.getSemanticConcepts();
    ...
    const insights: SemanticInsight[] = limited.map(concept => ({
      ...
      evolution: {
        ...
        changeCount: Array.isArray((concept.evolutionHistory as any)?.changes) ? (concept.evolutionHistory as any).changes.length : 0
      }
    }));
  ...
  async getPatternRecommendations(args: CodingContext & {
    ...
    const patterns = await this.storage.getDeveloperPatterns(undefined, 100);
  ...
  async getDeveloperProfile(args: {
    ...
    const patterns = await this.storage.getDeveloperPatterns(undefined, 50);
    ...
    const profile: DeveloperProfile = {
      preferredPatterns: patterns.slice(0, 10).map(p => ({
        pattern: p.patternId,
        description: String(p.patternContent.description ?? 'Developer pattern'),
        confidence: p.confidence,
        // Limit to 2 examples per pattern, truncate each to 150 chars
        examples: p.examples.slice(0, 2).map(ex => truncateCode(String(ex.code ?? ''))),
    ...
    if (args.includeWorkContext) {
      const projectPath = process.cwd();
      const session = await this.storage.getCurrentWorkSession(projectPath);

      if (session) {
        const decisions = await this.storage.getProjectDecisions(projectPath, 5);
        profile.currentWork = {
  ...
  async contributeInsights(args: AIInsights & {
    ...
    try {
      ...
      await this.storage.insertAIInsight({
        insightId,
  ...
  private async updateWorkSession(sessionUpdate: {
    ...
    let session = await this.storage.getCurrentWorkSession(projectPath);

    if (!session) {
      await this.storage.createWorkSession({
        id: nanoid(),
        projectPath,
      ...
      session = await this.storage.getCurrentWorkSession(projectPath);
    }
    ...
    if (session) {
      ...
      if (Object.keys(updates).length > 0) {
        await this.storage.updateWorkSession(session.id, updates);
      }
    }

    if (sessionUpdate.decisions) {
      for (const [key, value] of Object.entries(sessionUpdate.decisions)) {
        await this.storage.upsertProjectDecision({
          id: nanoid(),
          projectPath,
          decisionKey: key,
          decisionValue: value
  ...
  async getProjectBlueprint(args: { path?: string; includeFeatureMap?: boolean }): Promise<{
    ...
    const projectStorage = await SurrealStorage.create(projectDbPath);

    try {
      // Get entry points from database
      const entryPoints = await projectStorage.getEntryPoints(projectPath);
      const entryPointsMap = entryPoints.reduce((acc: Record<string, string>, ep) => {
        acc[ep.entryType] = ep.filePath;
        return acc;
      }, {} as Record<string, string>);

      // Get key directories from database
      const keyDirs = await projectStorage.getKeyDirectories(projectPath);
      const keyDirsMap = keyDirs.reduce((acc: Record<string, string>, dir) => {
      ...
      if (args.includeFeatureMap) {
        const features = await projectStorage.getFeatureMaps(projectPath);
        featureMap = features.reduce((acc: Record<string, string[]>, feature) => {
      ...
      const learningStatus = await this.getLearningStatus(projectStorage, projectPath);
   ...
   */
  private async getLearningStatus(storage: SurrealStorage, projectPath: string): Promise<{
    hasIntelligence: boolean;
    isStale: boolean;
    conceptsStored: number;
    patternsStored: number;
  ...
  private async checkExistingIntelligence(path: string): Promise<{ concepts: number; patterns: number } | null> {
    const concepts = (await this.storage.getSemanticConcepts()).length;
    const patterns = (await this.storage.getDeveloperPatterns()).length;
  ...
  private async checkExistingIntelligenceInDatabase(storage: SurrealStorage, path: string): Promise<{ concepts: number; patterns: number } | null> {
    const concepts = (await storage.getSemanticConcepts()).length;
    const patterns = (await storage.getDeveloperPatterns()).length;
  ...
  private async storeIntelligence(
    ...
    for (const concept of concepts) {
      await this.storage.insertSemanticConcept({
        id: concept.id,
    ...
    for (const pattern of patterns) {
      await this.storage.insertDeveloperPattern({
  ...
  private async storeProjectBlueprint(
    ...
    if (codebaseAnalysis.entryPoints && Array.isArray(codebaseAnalysis.entryPoints)) {
      for (const entryPoint of codebaseAnalysis.entryPoints) {
        await storage.insertEntryPoint({
          id: nanoid(),
          projectPath,
    ...
    if (codebaseAnalysis.keyDirectories && Array.isArray(codebaseAnalysis.keyDirectories)) {
      for (const directory of codebaseAnalysis.keyDirectories) {
        await storage.insertKeyDirectory({
          id: nanoid(),
          projectPath,
  ...
  private async buildSemanticIndex(concepts: any[], patterns: any[]): Promise<number> {
    try {
      ...
      for (const concept of concepts) {
        ...
        await this.storage.storeCodeEmbedding(text, {
          id: concept.id,
          filePath: concept.filePath,
