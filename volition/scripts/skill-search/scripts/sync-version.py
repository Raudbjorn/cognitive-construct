...
def sync_versions(check_only: bool = False) -> bool:
    ...
    updates: List[Tuple[str, Path, str, str]] = [
        ...
        (
            "Backend __init__.py",
            repo_root / "packages/backend/src/skill_search_backend/__init__.py",
            r'^__version__ = "[^"]+"',
            f'__version__ = "{version}"',
        ),
        (
            "Frontend __init__.py",
            repo_root / "packages/frontend/src/skill_search/__init__.py",
            r'^__version__ = "[^"]+"',
            f'__version__ = "{version}"',
        ),
        (
            "HTTP server health endpoint",
            repo_root / "packages/backend/src/skill_search_backend/http_server.py",
            r'"version": "[^"]+"',
            f'"version": "{version}"',
        ),
        (
            ...
            r'frontend_wheel = "packages/frontend/dist/skill_search-[0-9]+\.[0-9]+\.[0-9]+-py3-none-any\.whl"',
            f'frontend_wheel = "packages/frontend/dist/skill_search-{version}-py3-none-any.whl"',
        ),
        (
            "Integration test (backend wheel)",
            repo_root / "tests/integration/test_timing.py",
            r'"packages/backend/dist/skill_search_backend-[0-9]+\.[0-9]+\.[0-9]+-py3-none-any\.whl"',
            f'"packages/backend/dist/skill_search_backend-{version}-py3-none-any.whl"',
        ),
        (
            "GitHub workflow (backend wheel)",
            repo_root / ".github/workflows/test.yml",
            r'skill_search_backend-[0-9]+\.[0-9]+\.[0-9]+-py3-none-any\.whl',
            f'skill_search_backend-{version}-py3-none-any.whl',
        ),
        (
            "GitHub workflow (frontend wheel)",
            repo_root / ".github/workflows/test.yml",
            r'skill_search-[0-9]+\.[0-9]+\.[0-9]+-py3-none-any\.whl',
            f'skill_search-{version}-py3-none-any.whl',
        ),
    ]
    ...
    if check_only:
        ...
        for name, package_dir in packages:
            ...
            if lock_file.exists():
                ...
                if name == "Backend":
                    pattern = r'name = "skill-search-backend"\nversion = "([^"]+)"'
                else:
                    pattern = r'name = "skill-search"\nversion = "([^"]+)"'
