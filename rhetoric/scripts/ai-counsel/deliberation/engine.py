...
class DeliberationEngine:
    ...
    def __init__(
        ...
        Args:
            adapters: Dictionary mapping adapter names to HTTP adapter instances
        ...
        summarizer_preferences = [
            ("openrouter", "anthropic/claude-3.5-sonnet", "OpenRouter Claude Sonnet"),
            ("ollama", "llama2", "Ollama Llama2"),
            ("lmstudio", "default", "LM Studio"),
        ]

        for adapter_name, model_name, display_name in summarizer_preferences:
            if adapter_name in adapters:
                from deliberation.summarizer import DeliberationSummarizer

                self.summarizer = DeliberationSummarizer(adapters[adapter_name], model_name)
        ...
        if not self.summarizer:
            logger.warning(
                ...
                "Configure at least one HTTP adapter (openrouter, ollama, or lmstudio) for AI-powered summaries."
            )
