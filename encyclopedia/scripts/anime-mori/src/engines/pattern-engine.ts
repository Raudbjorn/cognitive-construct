...
import type { DeveloperPattern } from '../storage/types.js';
...
export class PatternEngine {
  ...
  constructor(private storage: SurrealStorage) {
  ...
  async findRelevantPatterns(
    ...
    const dbPatterns = await this.storage.getDeveloperPatterns(undefined, 200);
    ...
    const relevantPatterns = scoredPatterns
      ...
      .map(({ pattern, score }) => {
        ...
        return {
          ...
          frequency: pattern.frequency,
          contexts: pattern.contexts,
          examples: pattern.examples.map(ex => ({ code: String(ex.code ?? '') })),
          confidence: pattern.confidence
        };
      });
    ...
    try {
      ...
      if (analysisData.patterns && analysisData.patterns.detected) {
        for (const patternType of analysisData.patterns.detected) {
          const existingPatterns = await this.storage.getDeveloperPatterns(patternType);
  ...
  async getPatternsByType(patternType: string, limit?: number): Promise<DeveloperPattern[]> {
    return this.storage.getDeveloperPatterns(patternType, limit);
  }

  async getAllPatterns(limit?: number): Promise<DeveloperPattern[]> {
    return this.storage.getDeveloperPatterns(undefined, limit);
  }
  ...
  async getPatternStatistics(): Promise<{
    ...
    const allPatterns = await this.storage.getDeveloperPatterns(undefined, 100);
  ...
  async routeRequestToFiles(
    ...
    try {
      const featureMaps = await this.storage.getFeatureMaps(projectPath);
  ...
  async findFilesUsingPatterns(
    ...
    try {
      const allFileIntel = await this.storage.getFeatureMaps(projectPath);
